        ...
        var version = '0.2.1',
            ...
            updateAllSelected = function(statusmarker, change, selected) {
                for (var i in selected) {
                    ...
                    var oldCount = graphic.get('status_'+statusmarker) | 0;
                    log(oldCount)
                    graphic.set('status_'+statusmarker, oldCount + change);
                }
            },
            handleInput = function(msg) {
                ...
                //
                // for (var i in msg.selected){
                // 	var el = msg.selected[i];
                // 	log(el);
                // 	var graphic = getObj(el._type, el._id);
                // 	log(graphic);
                // 	log("status markers = " );
                // 	log(graphic.statusmarkers);
                ...
                _.each(args,function(s){
                    ...
                    // var matches = s.match(expectedForm);
                    log(s);
                    if (!matches){
                        return;
                    }
                    var toAdd = matches[1] == '+';
                    var numberToIncreaseBy = matches[2] | 0;
                    var statusToChange = matches[3];
                    log(statusToChange);
                    if(statusToChange.match(regex.statuses)) {
                        var delta = toAdd ? numberToIncreaseBy : numberToIncreaseBy * -1;
                        updateAllSelected(statusToChange, delta, msg.selected);
                    }
                });
